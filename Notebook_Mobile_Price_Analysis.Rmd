...
p1 <-  ggplot(dataSet, aes(x=blue, fill=blue)) +
  theme_bw() +
  geom_bar() +
  ylim(0, 1050) +
  labs(title = "Bluetooth") +
  scale_x_discrete(labels = c('Not Supported','Supported'))
p2 <- ggplot(dataSet, aes(x=dual_sim, fill=dual_sim)) +
  theme_bw() +
  geom_bar() +
  ylim(0, 1050) +
  labs(title = "Dual Sim") +
  scale_x_discrete(labels = c('Not Supported','Supported'))
p3 <- ggplot(dataSet, aes(x=four_g, fill=four_g)) +
  theme_bw() +
  geom_bar() +
  ylim(0, 1050) +
  labs(title = "4 G") +
...
```{r}
data = data.frame(MagaPixels = c(dataSet$fc, dataSet$pc), 
               Camera = rep(c("Front Camera", "Primary Camera"), c(length(dataSet$fc), length(dataSet$pc))))
...
```{r}
forPriceRange0 <- dataSet %>% filter(price_range == 0)
mean(forPriceRange0$battery_power)

forPriceRange1 <- dataSet %>% filter(price_range == 1)
mean(forPriceRange1$battery_power)

forPriceRange2 <- dataSet %>% filter(price_range == 2)
mean(forPriceRange2$battery_power)

forPriceRange3 <- dataSet %>% filter(price_range == 3)
mean(forPriceRange3$battery_power)
```
